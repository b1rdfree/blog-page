/* 来源：React Bits（reactbits.dev）TS 变体，MIT + Commons Clause。参数已按本站主题调整。 */
...
const LetterGlitch = ({
  ...
  const drawLetters = () => {
    // 【本地修复】上游这里用的是 canvasRef.current!（非空断言）。
    // 组件卸载后仍会有一次 rAF / resize 回调进来，此时 ref 已置空，
    // 会在 getBoundingClientRect 上抛 TypeError。改成取值 + 判空。
    const canvas = canvasRef.current;
  ...
  useEffect(() => {
    ...
    return () => {
      // 【本地修复】上游漏了 clearTimeout：handleResize 里有 100ms 防抖 setTimeout，
      // 若组件在防抖窗口内被卸载（切路由/改视口后立刻离开），
      // 回调仍会执行 resizeCanvas + animate，去操作已卸载的 canvas 并抛错。
      clearTimeout(resizeTimeout);
